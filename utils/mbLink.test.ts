import { type EntityWithUrlRels, getMusicBrainzEditLink } from './mbLink.ts';
...
describe('getMusicBrainzEditLink', () => {
	...
	it('returns null if entity has no externalIds', () => {
		...
		assertEquals(getMusicBrainzEditLink({ entity, entityType, sourceEntityUrl, providers: mockProviders }), null);
	});
	...
	it('returns null if all external links already exist', () => {
		...
		assertEquals(
			getMusicBrainzEditLink({ entity, entityType, sourceEntityUrl, entityCache, providers: mockProviders }),
			null,
		);
	});
	...
	it('returns a URL with correct search params for new links', () => {
		...
		const url = getMusicBrainzEditLink({ entity, entityType, sourceEntityUrl, entityCache, providers: mockProviders });
	...
	it('handles missing entityCache gracefully', () => {
		...
		const url = getMusicBrainzEditLink({ entity, entityType, sourceEntityUrl, providers: mockProviders });
		assertInstanceOf(url, URL);
		const searchParams = url!.searchParams;
