...
function ok(data: JSONVal) { return json(data, { status: 200 }); }
...
const routes: Record<string, Record<string, RouteHandler>> = {
  ...
  "/api/v1/lookup": {
    GET: async (_req, url) => {
      ...
      try {
        const result = urlParam 
          ? await lookupByUrl(urlParam, merge)
          : await lookupByGtin(gtinParam!);
  ...
  "/api/v1/match": {
    GET: async (_req, url) => {
      ...
      try {
        const { merged, providerMap } = urlParam 
          ? await lookupByUrl(urlParam, merge)
          : await lookupByGtin(gtinParam!);
